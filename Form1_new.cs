...
namespace PhylogeneticTree
{
    public partial class Form1 : Form
    {
        ...
        {
            //if (DateTime.Now > new DateTime(2015, 6, 29))
            //{
            ...
            timer1.Enabled = true;
            pnlLoad.Left = (tabControl1.ClientSize.Width - pnlLoad.Width) / 2;
            pnlLoad.Top = (tabControl1.ClientSize.Height - pnlLoad.Height) / 3;
            pnlWaiting.Left = (tabControl1.ClientSize.Width - pnlWaiting.Width) / 2;
            pnlWaiting.Top = (tabControl1.ClientSize.Height - pnlWaiting.Height) / 3;
            pbLoad.Left = (pnlLoad.ClientSize.Width - pbLoad.Width) / 2;
            pbLoad.Top = (pnlLoad.ClientSize.Height - pbLoad.Height) / 3;
            label6.Left = (pnlLoad.ClientSize.Width - label6.Width) / 2;
        ...
        private void ImprovedStepwiseAddition()
        {
            ...
            label1.Text = "Clustering species with S-Clustering algorithm"; label1.Refresh();
        ...
        private void load(bool _importToCurrent = false)
        {
            ...
            try
            {
                ...
                //
                //SEQ_LENGTH = isequence.Count();

                //sequenceArray = new string[isequence.Count()];

                /*
                for (int i = 0; i < isequence.Count(); i++)
                {
                    byte[] bArray = isequence.ElementAt(i).ToArray();
                    for (int j = 0; j < bArray.Length; j++)
                        if ((char)bArray[i] == '?' || (char)bArray[i] == 'n')
                            sequenceArray[i] += 'N';
                        else
                            sequenceArray[i] += (char)bArray[j];
                    ...
                    strarray[0] = i.ToString();
                    strarray[1] = isequence.ElementAt(i).Count().ToString();
                    strarray[2] = isequence.ElementAt(i).ID;
                }
                */
                ...
                //frmManageSequences frm = new frmManageSequences();
                //frm.isequence = isequence;
                //frm.SEQ_LENGTH = SEQ_LENGTH;
                //frm.ShowDialog();
            }
            catch
            {
                MessageBox.Show("System.OutOfMemoryException was thrown! Please exit the application and re-open it again. If the problem persists, you may need to restart your Windows.", "Fatal Error",
        ...
        private void open(bool _importToCurrent = false)
        {
            ...
            //pnlLoad.Visible = true;
            //pnlLoad.Refresh();
            string path = Application.StartupPath + "\\waitingForm.exe";
        ...
        {
            string path = Application.StartupPath + "\\processingForm.exe";
            System.Diagnostics.Process p = System.Diagnostics.Process.Start(path);
            Application.DoEvents();
        ...
        private void showSeqsInDGV(DataGridView dgv, List<ISequence> iSeqs, bool _importToCurrent = false)
        {
            if (!_importToCurrent)
            {
                ...
                dgv.Columns.Add("rowColumn", "#");
                dgv.Columns[1].Width = 20;
                dgv.Columns.Add("idColumn", "ID");
                dgv.Columns[2].Width = 120;
                dnaSequenceUtil dsc = new dnaSequenceUtil();
                numberOfDNAColumns = dsc.findMaxSequenceLength(iSeqs);
                for (int i = 0; i < numberOfDNAColumns; i++)
                {
                    dgv.Columns.Add("column" + (i + 1).ToString(), (i + 1).ToString());
                    dgv.Columns[i + 3].DefaultCellStyle.Font = new Font("Tahoma", 8.5F, FontStyle.Bold);
                    dgv.Columns[i + 3].Width = 35;
                    dgv.Columns[i + 3].FillWeight = 1;
                }
            }
            else
            {
                ...
                if (importedNumberOfColumns > numberOfDNAColumns)
                {
                    ...
                    {
                        dgv.Columns.Add("column" + (numberOfDNAColumns + i + 1).ToString(), (numberOfDNAColumns + i + 1).ToString());
                        dgv.Columns[numberOfDNAColumns + i + 3].DefaultCellStyle.Font = new Font("Tahoma", 8.5F, FontStyle.Bold);
                        dgv.Columns[numberOfDNAColumns + i + 3].Width = 35;
                        dgv.Columns[numberOfDNAColumns + i + 3].FillWeight = 1;
                    }
        ...
        {
            WriteToFile(Filename, isequence);
        }
